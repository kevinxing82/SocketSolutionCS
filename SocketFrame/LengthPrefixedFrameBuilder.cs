...
namespace org.kevinxing.socket
{
    ...
    public sealed class LengthPrefixedFrameBuilder : FrameBuilder
    {
        ...
        public LengthPrefixedFrameBuilder(LengthPrefixedFrameEncoder encoder, LengthPrefixedFrameDecoder decoder) : base(encoder, decoder)
        {
        }
    }
        ...
        private static  byte[] Encode(byte[] payload,int offset, int count, bool isMasked = false)
        {
            ...
            if(isMasked)
            {
                ...
                if(count>0)
                {
                    ...
                    {
                        fragment[payloadIndex + i] = (byte)(payload[offset + 1] ^ fragment[maskingKeyIndex + i % MaskingKeyLength]);
                    }
                }
            }
            else
